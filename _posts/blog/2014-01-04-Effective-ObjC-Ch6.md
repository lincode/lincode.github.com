...
重复调用 @synchronized(self) 是很危险的，因为所有这些块都会被彼此同步。如果多个属性这么做，每一个属性将会被和其它所有属性同步，这可能并不是你想要的。你真正想要的是每个属性的访问被独立同步。
...
这个方法族里最基本的是 performSelector:。它带一个参数，就是要执行的选择器，签名如下：
...
可是，这些方法是有限制的。例如，没有方法可以延迟执行有两个参数的选择器。基于同样的原因，线程方法也不是通用的。要使用这些方法的代码常常会将多个参数打包入一个字典，并在被调用方法中解包，因此这会增加开销和潜在的 bug。
...
除非你使用使用内建的 NSOperation 的具体子类，例如 NSBlockOperation，否则你必须创建自己的子类。这个类，作为一个普通的 Objective-C 对象，可以存储任何你想要的信息。当它运行时，它可以使用所有它存储的信息和使用所有该类定义的方法。这使得它比一个分发队列中的 block 要强大得多。顺应软件开发的 “Don‘t Repeat Yourself”（DRY）原则，这些操作类可以在你得代码里重用。
