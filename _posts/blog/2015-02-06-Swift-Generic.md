...
泛型编程允许定义参数化的类型，使得函数和类不再被限制在某种类型。泛型一般出现在强类型语言中，因为对于弱类型语言并无必要，而强类型语言对类型有更严格类型检查，需要泛型机制加强类型安全，减少类型转换的次数。本质来说，泛型是对类型的抽象，它带来了更高层次的抽象，避免了重复的代码。
...
除了泛型函数之外，Swift还允许为自定义泛型类型，这样的自定义泛型类型，结构体和枚举就可以作用于任何类型，和Swift已经提供的Array和Dictionary的用法相同。
...
之前所举的例子中的泛型函数和泛型结构体，都没有对泛型参数做约束。也就是说，函数`func swapTwoValues<T>(inout a: T, inout b: T)`和结构体`Stack<T>`中的，泛型参数T可以被用于代表任何类型。但是，编程实践中，我们会遇到一些场景中并不能适用所有的类型。这时就需要对泛型类型做一些约束。对泛型类型进行约束，保证了类型安全性。
