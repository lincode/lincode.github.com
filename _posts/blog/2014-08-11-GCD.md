...
NSOperation 代表了一个独立的计算单元。一般，我们会把计算任务封装进 NSOperation 这个对象。NSOperationQueue 则用于执行计算任务，管理计算任务的优先级，或者处理计算任务之间的依赖性。
...
GCD（Grand Central Dispatch）是 Apple 公司为了提高 OS X 和 iOS 系统在多核处理器上运行并行的代码的能力而开发的一系列相关技术，它提供了对线程的高级抽象。GCD 是 Apple 推荐的多线程编程工具。
