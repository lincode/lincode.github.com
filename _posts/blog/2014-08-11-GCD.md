...
本文梳理了 OS X 和 iOS 系统中提供的多线程工具。并且对这些工具的使用给出了一些实用的建议。
...
GCD(Grand Center Dispatch)是 Apple 公司为了提高 OS X 和 iOS 系统在多核处理器上运行并行的代码的能力而开发的一系列相关技术，它提供了对线程的高级抽象。GCD 是 Apple 推荐的多线程编程工具。下一节会详细介绍 GCD。

## GCD

GCD 是一套纯 C API，但是，它提供了简单易用并且功能强大的任务管理和多线程编程能力。GCD 需要和 blocks（Objective－C 的闭包）配合使用。block 是 GCD 执行单元。GCD 的任务需要被拆解到 blocks 中。block 被排入 GCD 的分发队列，GCD 会为你排期运行。GCD 创建，重用，销毁后台线程，基于系统资源以它认为合适的方式运行每个队列。

### GCD 分发队列

GCD 分发队列是执行任务的有力工具。分发队列你可以异步或者阻塞执行任意个 block 的代码。你可以使用分发队列来执行几乎任何线程任务。而且 GCD 提供了简单易用的接口。
...
GCD 有着丰富的功能，比如分发组（dispatch group），信号（semaphores），分发栅栏（dispatch barrier），分发源（dispatch source）等等。这些可以用于完成更复杂的并发任务。详细可以查阅 Apple 关于 GCD 的[文档](https://developer.apple.com/library/mac/documentation/performance/reference/gcd_libdispatch_ref/Reference/reference.html)。

## 使用建议

### 在能够使用 GCD 的地方，尽量选择 GCD

由于 Apple 公司宣称其在 GCD 技术中为更好地利用多核硬件系统做了很多的优化。所以，在性能方面 GCD 是不用担心的。而且 GCD 也提供了相当丰富的 API，几乎可以完成绝大部分线程相关的编程任务。下面举一些可以推荐使用 GCD 的例子：
