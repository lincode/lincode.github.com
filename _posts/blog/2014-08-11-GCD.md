...
这一系列方法简单易用，但只提供了有限的几个选择：可以指定是否需要阻塞当前线程等待结果；指定是在当前线程，还是在主线程，还是在后台线程执行；指定执行的方法（但传入方法的参数数量有限制）。

你可以指定方法的执行线程。比如，以下代码使得方法 foo 在一个新的后台线程执行，并传入了 object 参数：
...
提供一个两个人同时一起到烤箱抢面包的例子。我们启动两个线程，来代表两个人。由于 NSThread 不处理线程同步，你还需要一把同步锁（即类型为 NSLock 的实例变量 _lock），在 run 方法中会用到：
...
仍然用上一节 NSThread 中的两人抢面包的例子。由于计算任务没有变化，所以 run 方法并不用改变。这里使用 NSOperation 和 NSOperationQueue 来代表两个抢面包的人，并给予他们不同的优先级。由于 NSOperation 也不处理线程间同步问题，你仍然需要一把同步锁：
