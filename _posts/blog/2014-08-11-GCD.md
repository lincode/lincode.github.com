...
这一系列方法简单易用，但只提供了有限的几个选择：指定执行的方法（但传入方法的参数数量有限制）；指定是在当前线程，还是在主线程，还是在后台线程执行；可以指定是否需要阻塞当前线程等待结果。
...
提供一个两个人同时一起到烤箱抢面包的例子。我们启动两个线程，来代表两个人。由于 NSThread 不处理线程同步，你还需要一把同步锁（即类型为 NSLock 的实例变量 _lock），在 run 方法中会用到这把同步锁：
...
由于烤箱门比较小，同时只能有一个人去拿面包，这时 NSLock 就有了用武之地。所以，拿面包可以用下面的 run 方法表示：
...
NSOperation 代表了一个独立的计算单元。一般，我们会把计算任务封装进 NSOperation 这个对象。NSOperation 是抽象类，但同时也提供了两个可以直接使用的实体子类：NSInvocationOperation 和 NSBlockOperation。NSInvocationOperation 用于执行方法中的计算任务，NSBlockOperation 用于执行 block 中的计算任务。

NSOperationQueue 则用于执行计算任务，管理计算任务的优先级，处理计算任务之间的依赖性。NSOperation 被添加到 NSOperationQueue 中之后，队列会按优先级和进入顺序调度，NSOperation 对象会被自动执行。

仍然使用上一节 NSThread 中的两人抢面包的例子。由于计算任务没有变化，所以 run 方法并不改变。但这里需要使用 NSOperation 和 NSOperationQueue 来代表两个抢面包的人，并给予他们不同的优先级。由于 NSOperation 也不处理线程间同步问题，所以你仍然需要一把在 run 方法中会用到的同步锁：
...
NSOperation 提供以下任务优先级，设置 queuePriority 即可加快或者推迟操作的执行：
...
NSOperation 还有一个非常有用功能，就是“取消”。这是其他多线程工具（包括后面要讲到的 GCD）都没有的。调用 NSOperation 的 cancel: 方法即可取消该任务。当你知道这个任务没有必要再执行下去时，尽早安全地取消它肯定有利于节省系统资源的。

### GCD

GCD（Grand Central Dispatch）是 Apple 公司为了提高 OS X 和 iOS 系统在多核处理器上运行并行代码的能力而开发的一系列相关技术，它提供了对线程的高级抽象。GCD 是一整套技术包含了语音功能，运行时库，系统级别的优化，这些一起为并发代码的执行提供了系统级别的广泛优化。所以，GCD 也是 Apple 推荐的多线程编程工具。

GCD 是系统层面的技术，除了可以被系统级应用使用，也可以被更普通的高级应用使用。使用 GCD 之后，应用就可以轻松地在多核系统上高效运行并发代码，而不用考虑繁琐的底层问题。GCD 在系统层面工作，能很好地满足所有应用的并行运行需求，将可用资源平衡地分配给他们。

GCD 提供了一套纯 C API。但是，它提供的 API 简单易用并且有功能强大的任务管理和多线程编程能力。GCD 需要和 blocks（Objective－C 的闭包）配合使用。block 是 GCD 执行单元。GCD 的任务需要被拆解到 block 中。block 被排入 GCD 的分发队列，GCD 会为你排期运行。GCD 创建，重用，销毁后台线程，基于系统资源以它认为合适的方式运行每个队列。所以，用户需要关心的细节并不多。
